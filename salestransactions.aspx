<%@ Page Language="C#" AutoEventWireup="true" CodeFile="salestransactions.aspx.cs" Inherits="salestransactions" %>
...
<body class="body-back" style="margin:0px;width:100%;height:100%" onload="calitemvalues()"> <%--onload="loadLB()"--%>
		...
		<div class="phone-box wrap push" id="home" style="margin-right:0px">
			<div class="menu-notify" style="margin-right:0px">
				...
				<div class="Profile-mid">
					<h5 class="pro-link">Sales Editing</h5>
				</div>
     ...
     <asp:GridView ID="gvCart" CssClass="table" runat="server" AutoGenerateColumns="false" CellPadding="1" ForeColor="#333333" GridLines="None">
        ...
        <Columns>
            <asp:TemplateField HeaderText="Item Description" ItemStyle-Width="220px" ItemStyle-CssClass="Product">
                <ItemTemplate  >
                     <asp:TextBox ID="txtGProductDesc" runat="server" Text='<%# Eval("ProductDesc") %>' ></asp:TextBox>
                </ItemTemplate>
            </asp:TemplateField>
            <asp:TemplateField HeaderText="Qty" ItemStyle-Width="30px" ItemStyle-CssClass="qty">
                <ItemTemplate>
                     <asp:TextBox ID="txtGQty" runat="server" Text='<%# Eval("Qty") %>' ></asp:TextBox>
                </ItemTemplate>
            </asp:TemplateField>
             <asp:TemplateField HeaderText="Rate" ItemStyle-Width="80px" ItemStyle-CssClass="price" HeaderStyle-HorizontalAlign="Right"  ItemStyle-HorizontalAlign="Right">
                <ItemTemplate>
                    <asp:TextBox ID="txtGPrice" runat="server" Text='<%# Eval("SellPrice") %>' ></asp:TextBox>
                </ItemTemplate>
            </asp:TemplateField>
            <asp:TemplateField HeaderText="Action">
                <ItemTemplate>
                    <a href="#" id='<%# Eval("ProductDesc") %>' class="deletable" onclick="return RemoveRow(this)">Delete</a>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </asp:GridView>
      ...
      <div>
         <%-- <asp:GridView ID="givcart_duplicate" CssClass="table" runat="server" AutoGenerateColumns="false" CellPadding="1" ForeColor="#333333" GridLines="None">
        <AlternatingRowStyle BackColor="White"  />
        <EditRowStyle BackColor="#7C6F57" />
        <FooterStyle BackColor="#1C5E55" Font-Bold="True" ForeColor="White" />
        <HeaderStyle BackColor="#1C5E55" Font-Bold="True" ForeColor="White" />
        <RowStyle BackColor="#E3EAEB" />
        <Columns>
            <asp:TemplateField HeaderText="Item Description" ItemStyle-Width="220px" ItemStyle-CssClass="ProductD">
                <ItemTemplate>
                    <%# Eval("ProductDesc") %>
                </ItemTemplate>
            </asp:TemplateField>
            <asp:TemplateField HeaderText="Qty" ItemStyle-Width="30px" ItemStyle-CssClass="qtyD">
                <ItemTemplate>
                    <%# Eval("Qty")%>
                </ItemTemplate>
            </asp:TemplateField>
             <asp:TemplateField HeaderText="Rate" ItemStyle-Width="80px" ItemStyle-CssClass="priceD" HeaderStyle-HorizontalAlign="Right"  ItemStyle-HorizontalAlign="Right">
                <ItemTemplate>
                    <%# Eval("SellPrice")%>
                </ItemTemplate>
            </asp:TemplateField>
        ...
        function calitemvalues()
        {
           ...
           var gvDrv = document.getElementById("<%=gvCart.ClientID %>");
            ...
            for (i = 1; i < gvDrv.rows.length; i++) {
                ...
                alert(gvDrv.rows[i].cells[1].text);

                var gridviewrow = $(gvDrv.rows[parseInt(i)])
                alert(gridviewrow.find("TextBox[id*='txtGQty']").text);
             
                alert(gvDrv.rows[i].FindControl("txtGQty").value);
              ...
              //  sumPrice = sumPrice + parseFloat(cell[1].value);

                //cellProductDesc = gvDrv.Rows[i].FindControl("txtProductDesc")).Text.Trim();
                sumPrice = sumPrice + (parseFloat(gvDrv.rows[i].FindControl("txtQty").value) * parseFloat(gvDrv.rows[i].FindControl("txtPrice").value));
                alert(gvDrv.rows[i].FindControl("txtQty").value);
